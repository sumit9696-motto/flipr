...
$item_run=mysqli_query($conn,"INSERT INTO `order`(`itemname`, `itempic`, `price`, `disprice`, `category`, `iteminfo`, `cdate`, `up_date`, `userid`) VALUES ('$productname','$itempic','$productprice','$disprice','$categoryname','','','','$userid')");

// $item="SELECT * FROM order WHERE userid=$userid";
// $item="SELECT * FROM order WHERE userid=$userid";
$item="SELECT * FROM `order` WHERE userid=$userid";
            ...
            <div class="col-sm-8">
              <table >
                <tr>
                    ...
                    <th style="padding: 25px;">add Date</th>
            ...
            while($item_row = mysqli_fetch_array($item_run))
{
 ?>
                <tr>
                    ...
                    <td ><img src="admin/img/<?php echo $item_row[2] ?>" height="70px" width="100px" class="  clogo "></td>
                    <td><?php echo $item_row['category'];?></td>
                    <td><?php echo $item_row['cdate'];?></td>
                   
                   
                    <td><button class="btn btn-dark"><a href="orders.php" style="text-decoration: none;"class= "text-light";>Order</a></button></td>
...
if(isset($_POST['delete_btn'])){
  ...
  if($delete_query_run){
    echo"<script>alert('Iteam Delete successfuly ....');window.location.href='orders.php'</script>";
  }
  else{
